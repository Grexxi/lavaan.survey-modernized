...
out_file <- file.path(validation_dir, "ordinal_mi_complex.out")
if (!file.exists(out_file)) {
  stop(
    "Mplus output not found. Run `/Applications/MplusDemo/mpdemo ",
    "ordinal_mi_complex.inp` inside validation/mplus-demo first."
  )
}
...
if (!is.null(model$summaries)) {
  ...
  utils::write.csv(
    mplus_summary,
    file = file.path(validation_dir, "ordinal_mi_mplus_fit_summary.csv"),
    row.names = FALSE
  )

  lavaan_fit <- utils::read.csv(
    file.path(validation_dir, "ordinal_mi_lavaan_survey_fit.csv"),
  ...
  utils::write.csv(
    fit_comparison,
    file = file.path(validation_dir, "ordinal_mi_mplus_lavaan_fit_comparison.csv"),
    row.names = FALSE
  )
}
...
utils::write.csv(
  params,
  file = file.path(validation_dir, "ordinal_mi_mplus_parameters_raw.csv"),
  row.names = FALSE
)

lavaan_params <- utils::read.csv(
  file.path(validation_dir, "ordinal_mi_lavaan_survey_parameters.csv"),
  stringsAsFactors = FALSE
)
...
utils::write.csv(
  comparison,
  file = file.path(validation_dir, "ordinal_mi_mplus_lavaan_parameter_comparison.csv"),
  row.names = FALSE
)

message("Ordinal MI comparison written to: ",
        normalizePath(file.path(
          validation_dir,
          "ordinal_mi_mplus_lavaan_parameter_comparison.csv"
        )))
message("Ordinal MI fit comparison written to: ",
        normalizePath(file.path(
          validation_dir,
