...
# Create a moderate MAR/MCAR missingness pattern in three indicators. The same
# imputed datasets are then analyzed by lavaan.survey and Mplus, so the
# validation compares the analysis engines rather than different imputers.
dat_missing <- dat
p_y2 <- plogis(-1.40 + 0.45 * scale(dat$y1)[, 1] + 0.20 * (dat$str >= 4))
p_y5 <- plogis(-1.35 + 0.45 * scale(dat$y4)[, 1] - 0.15 * scale(dat$wgt)[, 1])
p_y6 <- rep(0.15, n)
...
imputed_data <- lapply(seq_len(m), function(i) {
  mice::complete(imp, action = i)
})

imputation_files <- sprintf("continuous_mi_imp%02d.dat", seq_len(m))
for (i in seq_along(imputed_data)) {
  utils::write.table(
    imputed_data[[i]][, c(items, "wgt", "clu", "str")],
...
writeLines(
  imputation_files,
  con = file.path(validation_dir, "continuous_mi_implist.dat")
)

writeLines(c(
  "TITLE: Continuous MI complex CFA for lavaan.survey validation;",
  "DATA:",
  "  FILE IS continuous_mi_implist.dat;",
  "  TYPE = IMPUTATION;",
  "VARIABLE:",
  ...
  "OUTPUT:",
  "  STDYX TECH1;"
), con = file.path(validation_dir, "continuous_mi_complex.inp"))
...
utils::write.csv(
  ...
  file = file.path(validation_dir, "continuous_mi_lavaan_survey_parameters.csv"),
  row.names = FALSE
)
...
utils::write.csv(
  ...
  file = file.path(validation_dir, "continuous_mi_lavaan_survey_fit.csv"),
  row.names = FALSE
)
...
utils::write.csv(
  missing_summary,
  file = file.path(validation_dir, "continuous_mi_missing_summary.csv"),
  row.names = FALSE
)

message("Continuous MI validation files written to: ", normalizePath(validation_dir))
message(
  "Next step: run `/Applications/MplusDemo/mpdemo ",
  "continuous_mi_complex.inp` inside that folder."
)
