...
if (!is.null(model$summaries)) {
  ...
  lavaan_fit <- utils::read.csv(
    file.path(validation_dir, "lavaan_survey_complex_fit.csv"),
  ...
  utils::write.csv(
    fit_comparison,
    file = file.path(validation_dir, "mplus_lavaan_fit_comparison.csv"),
    row.names = FALSE
  )
}
...
message("Fit comparison written to: ",
        normalizePath(file.path(validation_dir, "mplus_lavaan_fit_comparison.csv")))
