...
out_file <- file.path(validation_dir, "ordinal_group_complex.out")
if (!file.exists(out_file)) {
  stop(
    "Mplus output not found. Run `/Applications/MplusDemo/mpdemo ",
    "ordinal_group_complex.inp` inside validation/mplus-demo first."
  )
}
...
if (!is.null(model$summaries)) {
  ...
  utils::write.csv(
    mplus_summary,
    file = file.path(validation_dir, "ordinal_group_mplus_fit_summary.csv"),
    row.names = FALSE
  )

  lavaan_fit <- utils::read.csv(
    file.path(validation_dir, "ordinal_group_lavaan_survey_fit.csv"),
  ...
  fit_comparison <- data.frame(
    ...
    mplus_wlsmv = c(
      pick_one(mplus_summary, c("ChiSqM_Value", "ChiSqM_Mean")),
      ...
      pick_one(mplus_summary, c("RMSEA_Estimate", "RMSEA_Mean")),
...
utils::write.csv(
  params,
  file = file.path(validation_dir, "ordinal_group_mplus_parameters_raw.csv"),
  row.names = FALSE
)

lavaan_params <- utils::read.csv(
  file.path(validation_dir, "ordinal_group_lavaan_survey_parameters.csv"),
  stringsAsFactors = FALSE
)
...
message("Ordinal multiple-group parameter comparison written to: ",
        normalizePath(file.path(
          validation_dir,
          "ordinal_group_mplus_lavaan_parameter_comparison.csv"
        )))
message("Ordinal multiple-group fit comparison written to: ",
        normalizePath(file.path(
          validation_dir,
          "ordinal_group_mplus_lavaan_fit_comparison.csv"
        )))
