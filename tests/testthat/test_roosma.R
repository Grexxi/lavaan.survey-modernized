...
test_that("standard errors match", {
  ...
  expect_equal(ses_surv, ses_test_surv, tolerance = 1e-4)
  ...
  expect_equal(ses_wls, ses_test_wls, tolerance = 1e-5)

})

test_that("an estimate matches", {
  
  expect_equal(coef(fit.cfa.surv)['range~~range'], 1.892812, tolerance = 1e-4, check.attributes = FALSE)
  expect_equal(coef(fit.cfa.surv.wls.yb)['range~~range'], 2.369951, tolerance = 2e-4, check.attributes = FALSE) 
})
...
test_that("rep weights works", {
  ...
  expect_equal(ses, ses_exp, tolerance = 1e-4)
})
